     ...
     $.get("/gettaskdetail/" + taskid + "/").then(function(resp) {
         ...
         if (data.error_code == 0) {
             ...
             $.each(data.comments, function(i, item) {
                 var innerhtml = $("<div class='comment'><img class='smallphoto' src='/avatar/"+item.authorid+"/'><span class='comment-content' style='margin:5px'>" + item.content + "</span></div>");
 ...
 $("#add-comment-btn").click(function() {
     ...
     if (content !== "") {
         $.post("/addcomment/", {
             ...
             <span class='comment-content' style='margin:5px'>" + content + "</span></div>";
